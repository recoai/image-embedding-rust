...
use crate::image_transform::pipeline::{GenericTransform, ImageSize, MeanStdNormalization, ResizeRGBImage, ToArray, ToTensor, TransformationPipeline};
use crate::image_transform::utils::{model_filename, save_file_get};

pub type TractSimplePlan = SimplePlan<TypedFact, Box<dyn TypedOp>, Graph<TypedFact, Box<dyn TypedOp>>>;
    ...
    let model = tract_onnx::onnx()
        ...
        .with_input_fact(
            ...
            InferenceFact::dt_shape(f32::datum_type(), tvec!(
                1, 3, config.image_size.width, config.image_size.height
            )),
        )
        .unwrap()
        .into_optimized()
